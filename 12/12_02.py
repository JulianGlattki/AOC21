...
class CaveSystem:
    ...
    def get_paths(self, cave, current_path):
        ...
        visited_twice = False
        if any(x == 2 for x in [current_path.count(c) for c in current_path if c.is_small_cave]):
            visited_twice = True
        ...
        unvisited_connected_caves = cave.get_connected_caves(visited_twice)
        ...
        if len(list(filter(lambda x: x.name == cave.name, current_path))) == 1:
            cave.reset()
...
class Cave:
    ...
    def get_connected_caves(self, already_connected_twice):
        if already_connected_twice:
            return list(filter(lambda x: x.is_visited() is False and not x.is_start(), self.connected_caves))
        else:
            return list(filter(lambda x: not x.is_start(), self.connected_caves))
